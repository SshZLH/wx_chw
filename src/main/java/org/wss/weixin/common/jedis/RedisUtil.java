...
public class RedisUtil  {    
	...
	public Jedis getJedis(){
		return this.getJedis(ZkPropertyUtil.get("jedisServerIp"), Integer.parseInt(ZkPropertyUtil.get("jedisServerPort")));
	}
	/**
	 * 释放redis实例到连接池.
     * @param jedis redis实例
	...
	public void closeJedis(Jedis jedis){
		this.closeJedis(jedis, ZkPropertyUtil.get("jedisServerIp"), Integer.parseInt(ZkPropertyUtil.get("jedisServerPort")));
	}
	/**
	 * 释放连接池
	 * @param ip
	 * @param port
	 */
	public void destroyPool(String ip,int port){
		getPool(ip, port).destroy();
		if(maps.containsKey( ip + ":" + port)){
			maps.remove( ip + ":" + port);
		}
	}
}
