...
if (url.indexOf("?") != -1) { 
      ...
      memberIdTaoBao=strNamelist[1];
      var strid = strs[1];
      var strIdlist=strid.split("=");
      wahoceId=strIdlist[1];
}
...
function defaultMob(demo){
 ...
 //展示柜体信息   		
 $.ajax({
		url:"/orderHandlerAction/orderHandlerAction!placeOrderTaoBao.do?time="+(new Date()),
		type:'post',
		data:{
			wahoceId:wahoceId,
			memberIdTaoBao:memberIdTaoBao,
			status:status
		},
		success:function(msg){
			if(msg.result=="true"){
				...
				$(".address").html(josn.listwahoce[0].wahoName);
				wahoceSize=josn.listwahoce[0].wahoceSize;
				hourRenews=josn.listwahoce[0].hourRenew;
				dayRenews=josn.listwahoce[0].dayRenew;
				hourRenews=josn.listwahoce[0].hourRenew;
				dayRenews=josn.listwahoce[0].dayRenew;
				monthRenews=josn.listwahoce[0].monthRenew;
				hourStarts=josn.listwahoce[0].hourStart;
				dayStarts=josn.listwahoce[0].dayStart;
				monthStarts=josn.listwahoce[0].monthStart;
				$(".orderNumber").html(josn.listwahoce[0].wahoceNumber+"号柜");
				document.getElementById("wahoceIdInput").value=josn.listwahoce[0].wahoceId;
				wahoId=josn.listwahoce[0].wahoId;
				wahoType=josn.listwahoce[0].wahoType;
				priceLists=josn.priceList;
				adv1=josn.listwahoce[0].adv1;
				adv2=josn.listwahoce[0].adv2;
				adv3=josn.listwahoce[0].adv3;
				$('.price_des1').html(adv1);
				$('.price_des2').html(adv2);
				$('.price_des3').html(adv3);
				listvouchers = josn.listvoucher;
...
$(function(){
	...
	$(".payButton").bind('click',function(){
		...
		var memberName = $(".nameInput").val();
	 	document.getElementById("memberIdTaoBaoInput").value = memberIdTaoBao;
		var memberIdTaoBaoInput = document.getElementById("memberIdTaoBaoInput").value; 
		voucherId=defaultVoucherId;
		$.ajax({
			url:"orderHandlerAction!addImmediateViewTaoBao.do",
			type:'POST',
			data:{
				unit:unit,
				number:numberInput,
				...
				memberTel:memberTel,
				memberIdTaoBao:memberIdTaoBaoInput,
			...
			success:function(msg){
				var json=msg.info;
					...
					if(totals=="0.00" || totals=="0.0" || totals=="0" ){
						...
						updateOrderStatus(orderId,voucherId,memberIdTaoBaoInput);
					}else{
	...
	//修改订单状态
	function updateOrderStatus(orderId,voucherId,memberIdTaoBaoInput){
		$.ajax({
			type: "get",
			data:{"orderId":orderId,"voucherId":voucherId},
			url:"orderAction!updateOrderStatus.do?time="+(new Date()),
			success:function(msg){
				openDoor(orderId,memberIdTaoBaoInput);
			}
		});
	}
	//调用开锁功能
	function openDoor(orderId,memberIdTaoBaoInput){
			$.ajax({
			type: "get",
			...
			success:function(msg){
						 ...
						 window.location.href="/app/nwhc/taobao/openSuccess/index.html?cabinetDoorNo="+cabinetDoorNo+"&orderId="+orderId+"&memberIdTaoBao="+memberIdTaoBaoInput;
					},
				error:function(msg){
						 alert(msg);
					}
	...
	$("#bind_vau").on('click',function(){
		...
		if(voucherCode==""){
			...
			$.ajax({
				url:"warehouse/setVoucherTaoBao.do",
				type:'post',
				data:{
					memberIdTaoBao:memberIdTaoBao,
					//openId : openId,
